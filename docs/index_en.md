...
![Display of the upload area within the accepted task](screen2_en.png)
...
![Display of an overview of all already existing files in the folder](screen3_en.png)
...
| `folder` | With this parameter you can define where the upload of the files should take place. This parameter can occur repeatedly and thus allows an upload to several directories, between which the user can then choose. The values `master`, `main` and `media` always work. Any other value is an individual folder such as `photos` or `scans` and requires a matching entry `process.folder.images.photos={processtitle}_photos` in `goobi_config.properties`. Without that entry Goobi cannot resolve the folder and the plugin reports a configuration error instead of showing the upload area. |
